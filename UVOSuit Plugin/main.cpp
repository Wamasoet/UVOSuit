 ...
 * @return The resulting multiplied quaternion.
 */
 ...
 * @param roll The roll angle in radians (default: 0.0f).
 * @return The calculated quaternion representation.
 */
...
namespace UVOSuit {
    ...
    /**
     * @brief Configuration structure for the OpenXR API Layer Vignette.
     */
    ...
    /**
     * @brief Main settings container for the plugin's operational state.
     */
    struct UVOSettings {
        ...
        // Lens Mask API Layer Parameters
    ...
    /**
     * @brief Optimization cache to store real-time quaternion calculations.
     */
    ...
    public:
        ...
        /**
         * @brief Updates internal rotation caches. Designed for fast rendering during UI drag events.
         * @param config The current active UVOSettings configuration.
         */
    ...
    // Global instances explicitly contained within the namespace
    UVOSettings g_Config;
    OptimizationCache g_Cache;

    // C-Style Function Pointers for API Hooks
    ...
    struct DiagnosticState {
        DiagLevel level = DiagLevel::WARNING;
        std::string title = "Waiting for VR...";
        std::string details = "Hook xrLocateViews is not firing yet.";
    };

    DiagnosticState g_Diag;

    /**
     * @brief Helper function for fast diagnostic error reporting.
     * @param title The brief title of the diagnostic event.
     * @param details Detailed context regarding the event.
     * @param lvl The severity level of the diagnostic event.
     */
    void ReportError(const std::string& title, const std::string& details, DiagLevel lvl = DiagLevel::CRITICAL_ERR) {
        g_Diag.level = lvl;
        g_Diag.title = title;
        g_Diag.details = details;
    }
}
...
/**
 * @brief Hook for xrLocateViews to inject custom FOV and optical axis rotation calculations.
 */
MyXrResult Hook_xrLocateViews(MyXrSession session, const MyXrViewLocateInfo* viewLocateInfo, MyXrViewState* viewState, uint32_t viewCapacityInput, uint32_t* viewCountOutput, MyXrView* views) {
    ...
    // Validation ensures we avoid accessing invalid memory during early game initialization
    if (UVOSuit::g_Config.mod_enabled && UVOSuit::g_Config.enable_custom_fov && result == 0 && views != nullptr && viewCountOutput != nullptr && *viewCountOutput >= 2) {
        ...
        // 1. Apply Asymmetric FOV Scaling
        ...
        // 2. Apply Optical Center (Flat) Shifts
        ...
        // 3. Apply Optical Axis Rotation (Utilizing Optimization Cache)
...
MyXrResult Hook_xrGetInstanceProcAddr(MyXrInstance instance, const char* name, PFN_xrVoidFunction* function) {
    ...
    if (res == 0 && function && *function && name && strcmp(name, "xrLocateViews") == 0) {
        ...
        *function = (PFN_xrVoidFunction)Hook_xrLocateViews;
        ...
        // Update diagnostic state if the hook was successfully installed and no prior critical errors occurred
        if (UVOSuit::g_Diag.level != UVOSuit::DiagLevel::CRITICAL_ERR) {
            UVOSuit::g_Diag.level = UVOSuit::DiagLevel::OK;
            UVOSuit::g_Diag.title = "SUCCESS";
            UVOSuit::g_Diag.details = "UVOSuit Active and fully hooked into OpenXR pipeline.";
        }
    }
    return res;
}
...
FARPROC WINAPI Hook_GetProcAddress(HMODULE hModule, LPCSTR lpProcName) {
    ...
    // Memory pointer validation guard for legacy API compatibility
...
/**
 * @brief Retrieves the path to the global configuration file.
 */
...
/**
 * @brief Retrieves the path to the local (per-game) configuration file.
 */
...
/**
 * @brief Namespace housing helper functions for reading and writing INI configuration files.
 */
...
/**
 * @brief Applies convergence math to dynamically adjust 3D depth perception.
 * @tparam T Floating-point type parameter (float or double).
 * @param pitch Reference to the camera pitch angle.
 * @param yaw Reference to the camera yaw angle.
 * @param roll Reference to the camera roll angle.
 * @param boost_angle The angle degree to boost the convergence by.
 * @param is_left_eye Boolean flag indicating if the transformation targets the left eye.
 */
template <typename T>
...
/**
 * @brief The core UVOSuit plugin class interfacing with the UEVR framework.
 */
...
public:
    ...
    void on_initialize() override {
        ...
        // Initialize Shared Memory for Inter-Process Communication (IPC)
        ...
        if (UVOSuit::g_hMapFile_Vignette != nullptr) {
            ...
            UVOSuit::ReportError("IPC Failure", "CreateFileMapping failed. Error Code: " + std::to_string(err) + ". Lens Mask layer won't receive data.");
        }
    }
    ...
    void on_pre_engine_tick(API::UGameEngine* engine, float delta) override {
        // Determine target angle: if boost is enabled, use settings, otherwise target 0
        float target_angle = UVOSuit::g_Config.enable_3d_boost ? UVOSuit::g_Config.convergence_angle : 0.0f;

        // Smooth interpolation (speed 8.5f gives ~1 sec for a full transition)
        if (UVOSuit::g_Config.actual_convergence_angle != target_angle) {
            const float transition_speed = 8.5f;
            UVOSuit::g_Config.actual_convergence_angle += (target_angle - UVOSuit::g_Config.actual_convergence_angle) * std::min(1.0f, transition_speed * delta);

            // If the difference is microscopic, snap exactly to target to halt calculations
            if (std::abs(UVOSuit::g_Config.actual_convergence_angle - target_angle) < 0.005f) {
                UVOSuit::g_Config.actual_convergence_angle = target_angle;
            }
        }
...
private:
    ...
    void draw_interface() {
        ...
        if (ImGui::Begin("Ultimate VR Optics Suite", &m_display_menu, ImGuiWindowFlags_AlwaysAutoResize)) {
            ...
            // Sync UI state to the OpenXR API Layer via Shared Memory
            ...
            ImVec4 status_color;
            switch (UVOSuit::g_Diag.level) {
            case UVOSuit::DiagLevel::OK:           status_color = ImVec4(0.31f, 0.78f, 0.47f, 1.0f); break; // Green
            case UVOSuit::DiagLevel::WARNING:      status_color = ImVec4(0.96f, 0.82f, 0.38f, 1.0f); break; // Yellow
            case UVOSuit::DiagLevel::CRITICAL_ERR: status_color = ImVec4(0.9f, 0.2f, 0.2f, 1.0f);    break; // Red
            }

            ImGui::TextColored(status_color, "System Status: %s", UVOSuit::g_Diag.title.c_str());

            // Display detailed information if there's an error or warning
            if (UVOSuit::g_Diag.level != UVOSuit::DiagLevel::OK) {
                ImGui::PushStyleColor(ImGuiCol_ChildBg, ImVec4(0.12f, 0.12f, 0.12f, 0.5f));
                ImGui::PushStyleVar(ImGuiStyleVar_ChildRounding, 4.0f);

                ImGui::BeginChild("DiagDetails", ImVec2(0, 45), true, ImGuiWindowFlags_NoScrollbar);
                ImGui::TextWrapped("%s", UVOSuit::g_Diag.details.c_str());
                ImGui::EndChild();
